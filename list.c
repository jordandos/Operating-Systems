...
int compareSize(block_t *blk1, block_t *blk2) {
    return (blk1->end - blk1->start + 1) - (blk2->end - blk2->start + 1);
}

// Function to add a node to the list in ascending order based on physical address
void list_add_ascending_by_address(list_t *l, block_t *newblk) {
    node_t *new_node = malloc(sizeof(node_t));
    new_node->blk = newblk;
    new_node->next = NULL;

    if (l->head == NULL || l->head->blk->start > newblk->start) {
...
// Function to add a node to the list in ascending order based on block size
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk) {
    node_t *new_node = malloc(sizeof(node_t));
    new_node->blk = newblk;
    new_node->next = NULL;

    if (l->head == NULL || compareSize(new_node->blk, l->head->blk) <= 0) {
        new_node->next = l->head;
        l->head = new_node;
    } else {
        node_t *current = l->head;
        while (current->next != NULL && compareSize(new_node->blk, current->next->blk) > 0) {
            current = current->next;
        }
        new_node->next = current->next;
        current->next = new_node;
...
void list_coalesce_nodes(node_t **head) {
    ...
    while (current != NULL) {
        if (prev->blk->end == current->blk->start) {
...
bool list_is_in_by_pid(list_t *l, int pid) {
    ...
    while (current != NULL) {
        if (comparePid(current->blk, pid) == 0) {
            return true; // Found
        }
        current = current->next;
    }
