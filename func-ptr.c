...
{
    const Process p_this = (const Process)this;
    const Process p_that = (const Process)that;
    if (p_this -> priority > p_that -> priority) {
        return -1;
    }
    else if (p_this -> priority < p_that -> priority) {
    return 0;
        return 1;
    }
    else { //if priority is the same
        if (p_this -> arrival_time < p_that -> arrival_time) {
            return -1;
        }
        else if (p_this -> arrival_time > p_that -> arrival_time) {
            return 1;
        }
        else {
